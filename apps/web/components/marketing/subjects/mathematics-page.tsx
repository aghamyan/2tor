        ...
        <div className={styles.heroInner}>
          <div className={styles.heroCopy}>
            ...
            <p className={styles.heroPrompt}>
              Choose your child&apos;s grade to see the exact skills, topics, and learning milestones
            ...
            <div className={styles.heroActions}>
              ...
              <button className={styles.secondaryButton} type="button" onClick={(event) => openBooking(event.currentTarget)}>
                Book a free class
              </button>
            </div>
            <div className={styles.trustGrid} aria-label="Program assurances">
              <span><CircleCheck aria-hidden="true" /> Grade-aligned curriculum</span>
              <span><Target aria-hidden="true" /> Personalized learning plan</span>
              <span><UserRoundCheck aria-hidden="true" /> Verified mathematics tutors</span>
              <span><MessageSquareText aria-hidden="true" /> Feedback after every lesson</span>
            </div>
            <p className={styles.supportLine}>
              <Sparkles aria-hidden="true" /> Built for students catching up, staying on track, or moving ahead.
            </p>
          </div>
      ...
      <section className={styles.explorer} id="curriculum-explorer">
        <div className={styles.container}>
          <div className={styles.sectionHeading}>
            ...
            <p>See what students learn, why it matters, and how each level prepares them for what comes next.</p>
          </div>
                    ...
                    .map((level) => {
                      ...
                      return (
                        <button
                          ...
                          ref={(node) => { levelButtons.current[index] = node; }}
                          role="tab"
      ...
      <section className={styles.adaptiveSection}>
        <div className={styles.container}>
          <div className={styles.adaptiveCopy}>
            ...
            <p>
              A tutor can revisit prerequisites, teach the current course, or add stretch work—without
      ...
      <section className={styles.conversionSection} id="free-class">
        <div className={styles.conversionInner}>
          <div>
            ...
            <p>
              Tell us the level and goal. We&apos;ll match your child with a verified mathematics tutor
              for a focused free class.
            </p>
          </div>
          <div className={styles.conversionAction}>
            <button className={styles.lightButton} type="button" onClick={(event) => openBooking(event.currentTarget)}>
              Request a free class <MoveRight aria-hidden="true" />
            </button>
            <span><LockKeyhole aria-hidden="true" /> About one minute. No payment details.</span>
          </div>
        </div>
      </section>

      <button className={styles.mobileCta} type="button" onClick={(event) => openBooking(event.currentTarget)}>
        Book a free class <ArrowRight aria-hidden="true" />
      </button>

      {bookingOpen && (
        <BookingDialog
          initialLevel={selectedLevel.id}
...
function MathVisual({ level }: { level: CurriculumLevel }) {
  ...
  if (levelIndex <= 2) {
    return (
      <svg className={styles.mathVisual} viewBox="0 0 240 150" role="img" aria-label="Number blocks showing early number sense">
        <title>Number blocks</title>
        {[0, 1, 2, 3, 4, 5, 6].map((block) => (
          <rect className={styles.visualShape} height="31" key={block} rx="7" width="31" x={18 + (block % 4) * 39} y={30 + Math.floor(block / 4) * 39} />
        ))}
        <text className={styles.visualText} x="181" y="70">7</text>
        <text className={styles.visualFormula} x="18" y="132">4 + 3 = 7</text>
      </svg>
    );
  }
  if (levelIndex <= 4) {
    return (
      <svg className={styles.mathVisual} viewBox="0 0 240 150" role="img" aria-label="Fraction bar model">
        ...
        <text className={styles.visualFormula} x="20" y="124">1/2 = 2/4</text>
      </svg>
    );
  }
  if (levelIndex <= 8) {
    return (
      <svg className={styles.mathVisual} viewBox="0 0 240 150" role="img" aria-label="Coordinate plane with a linear graph">
        <title>Coordinate plane</title>
        <path className={styles.visualGrid} d="M35 20v105M75 20v105M115 20v105M155 20v105M195 20v105M20 35h195M20 70h195M20 105h195" />
  ...
  if (level.id === "geometry") {
    return (
      <svg className={styles.mathVisual} viewBox="0 0 240 150" role="img" aria-label="Triangle with a proof relationship">
        ...
        <text className={styles.visualFormula} x="133" y="82">a² + b² = c²</text>
      </svg>
    );
  }
  if (level.id === "precalculus") {
    return (
      <svg className={styles.mathVisual} viewBox="0 0 240 150" role="img" aria-label="Trigonometric function graph">
        <title>Function graph</title>
        <path className={styles.visualGrid} d="M20 35h200M20 75h200M20 115h200M50 20v110M110 20v110M170 20v110" />
  ...
  return (
    <svg className={styles.mathVisual} viewBox="0 0 240 150" role="img" aria-label="Algebraic function equation and graph">
      ...
      <text className={styles.visualFormula} x="113" y="129">y = 2x − 1</text>
    </svg>
  );
}
    ...
    >
      <div className={styles.container}>
        <div className={styles.curriculumIntro}>
          <div>
            <p className={styles.levelNumber}>
              Level {String(mathematicsCurriculum.findIndex((item) => item.id === level.id) + 1).padStart(2, "0")} of {mathematicsCurriculum.length}
            </p>
            <h2 aria-live="polite">{level.title}</h2>
            <p>{level.summary}</p>
          </div>
          <button className={styles.inlineBookButton} type="button" onClick={(event) => onBook(event.currentTarget)}>
        ...
        <div className={styles.bestFor}>
          ...
          {level.bestFor.map((item) => <span key={item}><Check aria-hidden="true" /> {item}</span>)}
        </div>
            ...
            return (
              <article className={styles.unit} data-expanded={expanded} key={curriculumUnit.id}>
                ...
                >
                  ...
                  <span className={styles.unitPosition}>Unit {index + 1} of {level.units.length}</span>
                ...
                {expanded && (
                  <div className={styles.unitPanel} id={panelId}>
                    <div>
                      ...
                      <ul>
                        {curriculumUnit.skills.map((skill) => <li key={skill}><Check aria-hidden="true" /> {skill}</li>)}
                      </ul>
                    </div>
                    <div className={styles.dependency}>
                      <span><ShieldCheck aria-hidden="true" /> Readiness checkpoint</span>
        ...
        <div className={styles.readinessGrid}>
          <div className={styles.outcomes}>
            ...
            <ul>
              {level.outcomes.map((outcome) => <li key={outcome}><CircleCheck aria-hidden="true" /> {outcome}</li>)}
            </ul>
          </div>
          <div className={styles.prerequisites}>
            ...
            <div className={styles.prerequisiteChips}>
              {level.prerequisites.map((skill) => <span key={skill}>{skill}</span>)}
            </div>
            <p>Not sure whether your child is ready?</p>
            <a href={`/${useLocale()}/free-assessment`}>Take a free placement assessment <ArrowRight aria-hidden="true" /></a>
          </div>
        </div>

        {nextLevel && (
          <div className={styles.nextLevel}>
            <div className={styles.nextMarker} aria-hidden="true"><ArrowRight /></div>
            <div>
              <p>What comes next</p>
  ...
  return (
    <div className={styles.dialogBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div
        ...
        role="dialog"
      >
        <button aria-label="Close booking form" className={styles.dialogClose} onClick={onClose} ref={closeButtonRef} type="button"><X aria-hidden="true" /></button>
        {status === "sent" ? (
          <div className={styles.formSuccess} role="status">
            ...
            <p id="booking-description">We&apos;ll email you to arrange a time and confirm the best starting point for {selectedLevel.shortTitle}.</p>
            <button className={styles.primaryButton} onClick={onClose} type="button">Done</button>
          </div>
        ) : (
          <>
            <div className={styles.dialogHeader}>
              ...
              <div className={styles.stepProgress} aria-label={`Step ${step} of 2`}>
                <span data-active={step >= 1}>1 <em>Learning fit</em></span>
                <span data-active={step >= 2}>2 <em>Your details</em></span>
              </div>
            </div>
            {step === 1 ? (
              <div className={styles.bookingStep}>
                <label className={styles.bookingField}>
                  ...
                  <select value={levelId} onChange={(event) => setLevelId(event.target.value)}>
                    {mathematicsCurriculum.map((level) => <option key={level.id} value={level.id}>{level.shortTitle}</option>)}
                  </select>
                </label>
                <fieldset className={styles.goalFieldset}>
                  ...
                  {supportGoals.map((item) => (
                    <label data-selected={goal === item.id} key={item.id}>
                      <input checked={goal === item.id} name="goal" onChange={() => setGoal(item.id)} type="radio" value={item.id} />
                      <span><strong>{item.label}</strong><small>{item.detail}</small></span>
                    </label>
                  ))}
                </fieldset>
                <button className={styles.primaryButton} onClick={() => setStep(2)} type="button">Continue <ArrowRight aria-hidden="true" /></button>
              </div>
            ) : (
              <form className={styles.bookingStep} onSubmit={submit}>
                <div className={styles.formSummary}><ClipboardCheck aria-hidden="true" /><span><strong>{selectedLevel.shortTitle}</strong>{selectedGoal.label}</span><button type="button" onClick={() => setStep(1)}>Edit</button></div>
                <label className={styles.bookingField}>
                ...
                <div className={styles.honeypot} aria-hidden="true"><label>Company<input autoComplete="off" name="company" tabIndex={-1} /></label></div>
                <label className={styles.formConsent}>
                  <input name="privacyConsent" required type="checkbox" />
                  <span>I agree that 2tor may contact me about this class. My details are handled according to the privacy policy.</span>
                </label>
                {status === "error" && <p className={styles.formError} role="alert">We couldn&apos;t send your request. Check your connection and try again.</p>}
                <div className={styles.formActions}>
                  <button className={styles.backButton} onClick={() => setStep(1)} type="button">Back</button>
                  <button className={styles.primaryButton} disabled={status === "sending"} type="submit">
                    {status === "sending" ? "Sending…" : "Request my free class"}
                  </button>
                </div>
                <p className={styles.privacyNote}><LockKeyhole aria-hidden="true" /> No payment details. No student name or birth date needed.</p>
              </form>
            )}
          </>
        )}
