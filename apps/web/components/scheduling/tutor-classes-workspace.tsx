      ...
      <header>
        ...
        <div>
          <button
            ...
            onClick={() => onSelect(new Date(anchor.getFullYear(), anchor.getMonth() - 1, 1))}
          >
            <ChevronLeft />
          </button>
          <button
...
function LessonPopover({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={`${studentName} lesson preview`}
    >
      <header>
        <span className={styles.avatar} data-color={subjectColor(lesson.subjectId)}>
          {initials(studentName)}
        </span>
        <div>
          <strong>{studentName}</strong>
          <small>Intermediate · Age 14</small>
        </div>
      ...
      <div className={styles.popoverSubject}>
        ...
        <div>
          ...
          <small>
            {formatTime(lesson.scheduledStartAt, timezone)}–
            {formatTime(lesson.scheduledEndAt, timezone)} · {minutesDuration(lesson)} min
          </small>
        </div>
      ...
      <div className={styles.popoverSignals}>
        <span>
          <BookOpenCheck /> Homework ready
        </span>
        <span>
          <UserRoundCheck /> 94% attendance
        </span>
        <span>
          <ArrowRight /> On track
        </span>
      ...
      <p className={styles.parentNote}>
        <MessageCircle /> Parent message <strong>“Could you review equations today?”</strong>
      </p>
      <footer>
        <Link className={styles.popoverPrimary} href={`/scheduling/${lesson.id}`}>
          Open lesson
        </Link>
        <Link href={`/scheduling/${lesson.id}#reschedule`}>Reschedule</Link>
        {lesson.status === "scheduled" ? (
          <button type="button" className={styles.popoverDanger} onClick={onDelete}>
            <Trash2 /> {lesson.lessonSeriesId ? "Delete series" : "Delete"}
          </button>
        ) : null}
      </footer>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className={styles.blockTop}>
        <strong>{subjectName}</strong>
        {lesson.lessonSeriesId ? <Repeat2 /> : null}
      </div>
      <span>{studentName}</span>
      <small>
        {formatTime(lesson.scheduledStartAt, timezone)}–
        {formatTime(lesson.scheduledEndAt, timezone)}
      </small>
      {!compact ? (
        <div className={styles.blockSignals}>
          <Video />
          <BookOpenCheck />
          <CheckCircle2 />
        </div>
...
function CalendarGrid({
  ...
  const visibleLessons = lessons.filter(
    (lesson) =>
      days.some((day) => sameDayInTimezone(lesson.scheduledStartAt, day, timezone)) &&
  ...
  function previewDrag(event: DragEvent<HTMLDivElement>) {
    ...
    const minutes = Math.max(
      0,
      Math.min(23 * 60 + 55, Math.round((((event.clientY - rect.top) / HOUR_HEIGHT) * 60) / 5) * 5),
    );
  ...
  return (
    <div className={styles.calendarFrame}>
      ...
      >
        ...
        {days.map((day) => (
          <button
            type="button"
            key={dateKey(day)}
            data-today={sameDay(day, displayNow)}
      ...
      <div className={styles.allDayRow} style={{ "--calendar-days": days.length } as CSSProperties}>
        ...
        {days.map((day) => (
          <div key={dateKey(day)}>
            {day.getDay() === 3 ? <em>Office hours · 2:00–3:00</em> : null}
          </div>
        ))}
      </div>
      <div className={styles.calendarScroll} ref={scrollRef}>
        ...
        <div
          ...
          onDrop={dropLesson}
          onDragLeave={(event) => {
            if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragPreview(null);
          }}
          onDoubleClick={(event) => {
            const rect = event.currentTarget.getBoundingClientRect();
            ...
            const minute = Math.max(
              0,
              Math.round(((event.clientY - rect.top) / HOUR_HEIGHT) * 12) * 5,
            );
        ...
        >
          ...
          {visibleLessons.map((lesson) => {
            const dayIndex = days.findIndex((day) =>
              sameDayInTimezone(lesson.scheduledStartAt, day, timezone),
            );
          ...
          {dragPreview ? (
            ...
            >
              <span>
                {formatTime(
                  new Date(
                    dragPreview.day.getFullYear(),
                    dragPreview.day.getMonth(),
                    dragPreview.day.getDate(),
                    0,
                    dragPreview.minutes,
                  ),
                )}
          ...
          {days.some((day) => sameDay(day, displayNow)) ? (
            <div
              ...
              style={
                {
                  "--now-day": days.findIndex((day) => sameDay(day, displayNow)),
                  "--now-top": `${(minutesFromMidnight(displayNow) / 60) * HOUR_HEIGHT}px`,
                  "--calendar-days": days.length,
                } as CSSProperties
              }
            >
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={dateKey(day)}
              data-outside={day.getMonth() !== anchor.getMonth()}
              data-today={sameDay(day, new Date())}
              onDoubleClick={() => onCreate(day)}
            >
              <button type="button" onClick={() => onCreate(day)}>
                {day.getDate()}
              </button>
              {dayLessons.slice(0, 3).map((lesson) => (
                ...
                >
                  <span>{formatTime(lesson.scheduledStartAt, timezone)}</span>
                  <strong>{subjectFor(lesson)}</strong>
                  <small>{studentFor(lesson)}</small>
                </button>
              ))}
              {dayLessons.length > 3 ? (
                <button type="button" className={styles.moreLessons}>
...
function AgendaList({
  ...
  const grouped = lessons
    ...
    .reduce<Map<string, CalendarLesson[]>>(
      (map, lesson) =>
        map.set(dateKeyInTimezone(lesson.scheduledStartAt, timezone), [
          ...(map.get(dateKeyInTimezone(lesson.scheduledStartAt, timezone)) ?? []),
          lesson,
        ]),
      new Map(),
    );
    ...
    <div className={styles.agendaList}>
      {[...grouped.entries()].map(([key, items]) => {
        ...
        return (
          <section key={key}>
            <header>
              <span>{zonedParts(first.scheduledStartAt, timezone).day}</span>
              <div>
                <strong>
                  {new Intl.DateTimeFormat(undefined, {
                    weekday: "long",
                    timeZone: timezone,
                  }).format(first.scheduledStartAt)}
                </strong>
                <small>
                  {new Intl.DateTimeFormat(undefined, {
                    month: "long",
            ...
            <div>
              {items.map((lesson) => (
                <button type="button" key={lesson.id} onClick={() => onPreview(lesson)}>
                  <span
                  ...
                  <time>
                    {formatTime(lesson.scheduledStartAt, timezone)}
                    <small>{minutesDuration(lesson)} min</small>
                  </time>
                  <span className={styles.avatar} data-color={subjectColor(lesson.subjectId)}>
                    {initials(studentFor(lesson))}
                  </span>
                  <div>
                    <strong>{studentFor(lesson)}</strong>
                    <small>
                      {subjectFor(lesson)} · {lesson.lessonSeriesId ? "Recurring" : "One-time"}
                    </small>
                  </div>
...
function PremiumList({
  ...
  const groups = [
    [
      "Today",
      (lesson: CalendarLesson) => sameDayInTimezone(lesson.scheduledStartAt, now, timezone),
    ],
    [
      "Tomorrow",
      (lesson: CalendarLesson) => sameDayInTimezone(lesson.scheduledStartAt, tomorrow, timezone),
    ],
    [
      "This week",
      (lesson: CalendarLesson) => {
    ...
    [
      "Later",
      (lesson: CalendarLesson) => dateInTimezone(lesson.scheduledStartAt, timezone) >= weekEnd,
    ],
  ] as const;
  return (
          ...
          <section key={label}>
            ...
            <div>
              {items.map((lesson) => (
                <article
                  key={lesson.id}
                  data-selected={selectedIds.has(lesson.id)}
                  onClick={(event) => onSelect(lesson, event)}
                  onDoubleClick={() => onPreview(lesson)}
                >
                  <input
                    type="checkbox"
                    readOnly
                    checked={selectedIds.has(lesson.id)}
                    aria-label={`Select ${studentFor(lesson)} lesson`}
                  />
                  <span className={styles.avatar} data-color={subjectColor(lesson.subjectId)}>
                    {initials(studentFor(lesson))}
                  </span>
                  <div className={styles.listStudent}>
                    ...
                    <small>
                      {subjectFor(lesson)} ·{" "}
                      {lesson.lessonSeriesId ? "Weekly lesson" : "One-time lesson"}
                    </small>
                  </div>
                  <time>
                    <strong>
                    ...
                    <small>
                      {formatTime(lesson.scheduledStartAt, timezone)} · {minutesDuration(lesson)}{" "}
                      min
                    </small>
                  </time>
                  <div className={styles.listSignals}>
                    <span>
                      <BookOpenCheck /> Homework
                    </span>
                    <span>
                      <UserRoundCheck /> Attendance
                    </span>
                  </div>
                  <StatusPill status={lesson.status} />
                  {lesson.joinUrl ? (
                    ...
                    <Link
                      href={`/scheduling/${lesson.id}`}
                      onClick={(event) => event.stopPropagation()}
                    >
                      Open
                    </Link>
                  )}
                  <button type="button" aria-label="More lesson actions">
                    <MoreHorizontal />
                  </button>
                </article>
              ))}
...
export function TutorClassesWorkspace({
  ...
  const [lessons, setLessons] = useState<CalendarLesson[]>(() =>
    initialLessons.map((lesson) => ({
      ...lesson,
      scheduledStartAt: new Date(lesson.scheduledStartAt),
      scheduledEndAt: new Date(lesson.scheduledEndAt),
    })),
  );
  ...
  const [moveRequest, setMoveRequest] = useState<{ lesson: CalendarLesson; date: Date } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (savedWorkspace === "calendar" || savedWorkspace === "list")
      setWorkspaceView(savedWorkspace);
    if (["day", "week", "month", "agenda"].includes(savedCalendar ?? ""))
      setCalendarView(savedCalendar as CalendarView);
    setTimezone(detectBrowserTimezone());
    /* eslint-enable react-hooks/set-state-in-effect */
  }, []);

  useEffect(() => {
    window.localStorage.setItem(VIEW_STORAGE_KEY, workspaceView);
  }, [workspaceView]);
  useEffect(() => {
  ...
  const studentFor = useCallback(
    (lesson: CalendarLesson) =>
      assignmentById.get(lesson.tutorStudentAssignmentId)?.studentName ?? "Student",
    [assignmentById],
  );
  const subjectFor = useCallback(
    (lesson: CalendarLesson) =>
      subjectById.get(lesson.subjectId) ??
      assignmentById.get(lesson.tutorStudentAssignmentId)?.subjectName ??
      lesson.subjectId.replaceAll("_", " "),
    [assignmentById, subjectById],
  );
  ...
  const filteredLessons = useMemo(
    () =>
      lessons.filter((lesson) => {
        const haystack =
          `${studentFor(lesson)} ${subjectFor(lesson)} ${lesson.status}`.toLowerCase();
        return (
          (!search || haystack.includes(search.toLowerCase())) &&
          (!selectedSubjects.size || selectedSubjects.has(lesson.subjectId)) &&
          (!recurringOnly || Boolean(lesson.lessonSeriesId))
        );
      }),
  ...
  const todayLessons = filteredLessons
    .filter(
      (lesson) =>
        sameDayInTimezone(lesson.scheduledStartAt, todayInTimezone, timezone) &&
        lesson.status === "scheduled",
    )
    .sort((a, b) => a.scheduledStartAt.getTime() - b.scheduledStartAt.getTime());
  const upcomingHomework = Math.max(
    2,
    Math.min(7, filteredLessons.filter((lesson) => lesson.status === "scheduled").length),
  );
  ...
  async function moveLesson(lesson: CalendarLesson, date: Date, scope = "this lesson") {
    ...
    try {
      ...
      setLessons((current) => current.map((item) => previousById.get(item.id) ?? item));
  ...
  async function undoMove() {
    ...
    try {
      ...
      setToast({
        message:
          "The move was saved, but undo could not be completed. Refresh to see the latest schedule.",
      });
    }
  }

  async function deleteLessonOrSeries(lesson: CalendarLesson) {
    const isSeries = Boolean(lesson.lessonSeriesId);
    const confirmed = window.confirm(
      isSeries
        ? "Delete the entire recurring series, including every upcoming class? This can't be undone."
        : "Delete this class? This can't be undone.",
    );
    if (!confirmed) return;
    ...
    try {
      const response = await fetch(
        isSeries
          ? `/api/scheduling/series/${lesson.lessonSeriesId}`
          : `/api/scheduling/lessons/${lesson.id}`,
        { method: "DELETE" },
      );
      ...
      setLessons((current) =>
        isSeries
          ? // Mirrors the server's own filter (services.ts `deleteLessonSeries`): only future,
            // still-scheduled occurrences are actually removed — a past one stuck at "scheduled"
            // survives, so drop it from local state too or it'd look deleted when it isn't.
            current.filter(
              (item) =>
                !(
                  item.lessonSeriesId === lesson.lessonSeriesId &&
                  item.status === "scheduled" &&
                  item.scheduledStartAt > now
                ),
            )
      ...
      setToast({ message: isSeries ? "Recurring series deleted" : "Class deleted" });
    } catch {
      setToast({ message: "The class could not be deleted. Try again." });
    }
  }
  ...
  return (
    <div className={styles.workspace}>
      <header className={styles.workspaceHeader}>
        <div>
          <h1>Classes</h1>
          <p>Manage your teaching schedule, recurring lessons, attendance and homework.</p>
        </div>
        <div className={styles.headerActions}>
          ...
          >
            ...
            {selectedSubjects.size ? <span>{selectedSubjects.size}</span> : null}
          </button>
        </div>
      </header>
      ...
      <div className={styles.primaryNav}>
        <div className={styles.segmented}>
          <button
            type="button"
            data-active={workspaceView === "calendar"}
          ...
          <button
            type="button"
            data-active={workspaceView === "list"}
        ...
        <p>
          ...
          {todayLessons.length ? `${todayLessons.length} lessons today` : "No lessons today"}
          <small>Times shown in {timezone.split("/").at(-1)?.replaceAll("_", " ")}</small>
        </p>
      </div>
      ...
      <div className={styles.calendarToolbar}>
        <div className={styles.toolbarLeft}>
          ...
          <button
            ...
            onClick={() => setAnchor(dateInTimezone(new Date(), timezone))}
          >
            Today
          </button>
          <div className={styles.arrowButtons}>
            <button type="button" aria-label="Previous period" onClick={() => navigate(-1)}>
              <ChevronLeft />
            </button>
            <button type="button" aria-label="Next period" onClick={() => navigate(1)}>
              <ChevronRight />
            </button>
          </div>
        ...
        <div className={styles.toolbarRight}>
          <div className={styles.timezoneControl}>
            <button type="button" onClick={() => setTimezoneMenuOpen(!timezoneMenuOpen)}>
              <Globe2 />
              <span>{timezone.split("/").at(-1)?.replaceAll("_", " ")}</span>
              <ChevronDown />
            </button>
            {timezoneMenuOpen ? (
              <div>
                {getTimezoneOptions().map((option) => (
                  <button
                    type="button"
                    key={option.value}
                    data-active={timezone === option.value}
                    onClick={() => {
                  ...
                  >
                    ...
                    {timezone === option.value ? <Check /> : null}
                  </button>
                ))}
              </div>
            ) : null}
          </div>
          <button type="button" className={styles.legendButton}>
            <span data-color="indigo" />
            <span data-color="teal" />
            <span data-color="violet" />
            <em>Color by subject</em>
          </button>
          <Link href="/scheduling/new" className={styles.createButton}>
            <Plus /> Create lesson
          </Link>
        </div>
      </div>
      ...
      <div className={styles.workspaceBody} data-sidebar={sidebarOpen}>
        {sidebarOpen ? (
          <aside className={styles.calendarSidebar}>
            ...
            <section className={styles.dayQueue}>
              <header>
                <div>
              ...
              {todayLessons.length ? (
                <ol>
                  {todayLessons.map((lesson, index) => (
                    <li key={lesson.id}>
                      <time>{formatTime(lesson.scheduledStartAt, timezone)}</time>
                      <span data-color={subjectColor(lesson.subjectId)} />
                      <div>
                        <strong>{studentFor(lesson)}</strong>
                        <small>
                          {subjectFor(lesson)} · {minutesDuration(lesson)} min
                        </small>
                      </div>
                ...
                <div className={styles.clearDay}>
                  <CheckCircle2 />
                  <p>
                    <strong>Clear teaching day</strong>
                    <span>
                      Your next lesson is{" "}
                      {filteredLessons[0]
                        ? new Intl.DateTimeFormat(undefined, {
                            ...
                            minute: "2-digit",
                            timeZone: timezone,
                          }).format(filteredLessons[0].scheduledStartAt)
                        : "not scheduled"}
                      .
                    </span>
                  </p>
            ...
            <section className={styles.attentionList}>
              ...
              <button type="button">
                <span className={styles.attentionIcon} data-tone="amber">
                  <UserRoundCheck />
                </span>
                <p>
                  <strong>Pending attendance</strong>
                  <small>2 lessons need a record</small>
                </p>
                <span>2</span>
              </button>
              <button type="button">
                <span className={styles.attentionIcon} data-tone="blue">
                  <BookOpenCheck />
                </span>
                <p>
                  <strong>Homework due soon</strong>
                  <small>{upcomingHomework} student tasks</small>
                </p>
                <span>{upcomingHomework}</span>
              </button>
              <button type="button">
                <span className={styles.attentionIcon} data-tone="violet">
                  <MessageCircle />
                </span>
                <p>
                  <strong>Parent messages</strong>
                  <small>One unread update</small>
                </p>
                <span>1</span>
              </button>
            </section>
            <section className={styles.quickFilters}>
              ...
              <Toggle
                ...
                onChange={() => setRecurringOnly(!recurringOnly)}
        ...
        <main className={styles.calendarMain}>
          {selectedIds.size > 1 ? (
            <div className={styles.bulkBar}>
              <strong>{selectedIds.size} lessons selected</strong>
              <button type="button">
                <Clock3 /> Move
              </button>
              <button type="button">
          ...
          {workspaceView === "list" ? (
            ...
            <MonthCalendar
              ...
              onCreate={(date) => {
                ...
                window.location.href = "/scheduling/new";
              }}
            />
          ) : calendarView === "agenda" ? (
            <AgendaList
        ...
        <div className={styles.drawerBackdrop} onMouseDown={() => setFiltersOpen(false)}>
          <aside className={styles.filterDrawer} onMouseDown={(event) => event.stopPropagation()}>
            ...
            <label className={styles.drawerSearch}>
              <Search />
              <input
                placeholder="Find a student or subject"
                value={search}
                onChange={(event) => setSearch(event.target.value)}
              />
            </label>
              ...
              {subjects.length ? (
                subjects.map((subject) => (
                  <label key={subject.id}>
                    ...
                    />
                    <span data-color={subjectColor(subject.id)} />
                    {subject.name}
                    <em>{lessons.filter((lesson) => lesson.subjectId === subject.id).length}</em>
                  </label>
                ))
              ) : (
                <p>No subject filters are available yet.</p>
              )}
            </section>
            <section>
              <h3>Lesson type</h3>
              <Toggle
                checked={recurringOnly}
                onChange={() => setRecurringOnly(!recurringOnly)}
                label="Recurring lessons only"
              />
              <Toggle checked={false} onChange={() => undefined} label="Homework assigned" />
              <Toggle checked={false} onChange={() => undefined} label="Assessment due" />
            </section>
            <footer>
              <button
                type="button"
              ...
              <button type="button" onClick={() => setFiltersOpen(false)}>
                Show {filteredLessons.length} lessons
              </button>
            </footer>
          </aside>
        </div>
      ...
      {previewLesson ? (
        <div className={styles.previewBackdrop} onMouseDown={() => setPreviewLesson(null)}>
          <div onMouseDown={(event) => event.stopPropagation()}>
            <LessonPopover
              lesson={previewLesson}
              studentName={studentFor(previewLesson)}
              subjectName={subjectFor(previewLesson)}
              timezone={timezone}
              onClose={() => setPreviewLesson(null)}
              onDelete={() => void deleteLessonOrSeries(previewLesson)}
            />
          </div>
        </div>
      ) : null}
      ...
      {moveRequest ? (
        <div className={styles.modalBackdrop}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="series-move-title"
          >
            <span className={styles.modalIcon}>
              <Repeat2 />
            </span>
            <h2 id="series-move-title">Move recurring lesson?</h2>
            <p>
              {studentFor(moveRequest.lesson)} · {subjectFor(moveRequest.lesson)}
              <br />
              New time:{" "}
              {new Intl.DateTimeFormat(undefined, {
                weekday: "long",
                ...
                minute: "2-digit",
                timeZone: timezone,
              }).format(moveRequest.date)}
            </p>
            <div>
              <button
                type="button"
                onClick={() => void moveLesson(moveRequest.lesson, moveRequest.date, "this lesson")}
              >
                Only this lesson<span>Keep the rest of the series unchanged</span>
              </button>
              <button
                type="button"
                onClick={() =>
                  void moveLesson(moveRequest.lesson, moveRequest.date, "this and future lessons")
                }
              >
                This and future lessons<span>Update upcoming occurrences</span>
              </button>
              <button
                type="button"
                onClick={() =>
                  void moveLesson(moveRequest.lesson, moveRequest.date, "the entire series")
                }
              >
                Entire series<span>Move every occurrence in the series</span>
              </button>
            </div>
            <button
              type="button"
      ...
      {toast ? (
        <div className={styles.toast} role="status">
          ...
          {toast.undo?.length ? (
            <button type="button" onClick={() => void undoMove()}>
              Undo
            </button>
          ) : null}
